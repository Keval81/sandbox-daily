...
export async function generateStaticParams() {
  return getArticlesByVertical("spotlights").map((a) => ({ slug: a.slug }));
}

export default async function SpotlightArticlePage({ params }: Props) {
  const { slug } = await params;
  const article = getArticleBySlug("spotlights", slug);
  if (!article) notFound();
  ...
  const config = verticals.spotlights;
  const related = getArticlesByVertical("spotlights")
    ...
    <>
        ...
        <div className="mx-auto max-w-[1440px] grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-12">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24">
              {article.subjectName && (
                <div className="mb-8">
                  <p className="font-mono text-meta-sm uppercase tracking-mono-wide text-grey mb-3">Subject</p>
                  <p className="font-display text-2xl font-bold text-ink leading-headline">
                    {article.subjectName}
                  </p>
                </div>
              )}
      ...
      {related.length > 0 && (
        <section className="bg-ink py-16 px-6">
          <div className="mx-auto max-w-[1440px]">
            <h2 className="font-mono text-meta uppercase tracking-mono-wide text-cream mb-8">More Spotlights</h2>
