...
export async function generateStaticParams() {
  return getArticlesByVertical("news").map((a) => ({ slug: a.slug }));
}
...
export default async function NewsArticlePage({ params }: Props) {
  ...
  const config = verticals.news;
  const related = getArticlesByVertical("news")
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {article.tags.map((tag) => (
                    <span key={tag} className="font-mono text-meta-sm uppercase tracking-mono text-orange border border-orange px-2 py-1 rounded-sharp">
                      {tag}
                    </span>
                  ))}
                </div>
