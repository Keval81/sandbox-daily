...
export async function generateStaticParams() {
  return getArticlesByVertical("sport").map((a) => ({ slug: a.slug }));
}

export default async function SportArticlePage({ params }: Props) {
  const { slug } = await params;
  const article = getArticleBySlug("sport", slug);
  if (!article) notFound();

  const htmlContent = await renderMarkdown(article.content);
  const config = verticals.sport;
  const related = getArticlesByVertical("sport")
    ...
    <>
      <section className={`${config.bg} ${config.text} py-20 px-6`}>
        <div className="mx-auto max-w-[1440px]">
          ...
          <ArticleSignals slug={article.slug} title={article.title} vertical="sport" placement="top" />
        </div>
      </section>
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {article.tags.map((tag) => (
                    <span key={tag} className="font-mono text-meta-sm uppercase tracking-mono text-green border border-green px-2 py-1 rounded-sharp">
                      {tag}
                    </span>
                  ))}
                </div>
